...
def sp_noise(image, prob):
    ...
    if len(image.shape) == 2:
        ...
        if colorspace == 3:  # RGB
            black = numpy.array([0, 0, 0], dtype='uint8')
            white = numpy.array([255, 255, 255], dtype='uint8')
        else:  # RGBA
            black = numpy.array([0, 0, 0, 255], dtype='uint8')
            white = numpy.array([255, 255, 255, 255], dtype='uint8')
    probs = numpy.random.random(image.shape[:2])
